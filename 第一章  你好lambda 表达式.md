...
#### 实施策略
政策规定的企业级程序。举例来说，我们也许必须要确保一个操作拥有安全认证。我们必须确保交易的快速进行，并且合适的更新交易的轨迹。这些任务通常就变成了普通的服务层叠代码，就像下面的过程代码一样。
``` java
...
```
这种方法有两个问题。第一，经常导致重复的工作。反过来就是，增加了维护的成本。这样会导致非常容易忘记在应用中抛出的异常，因此而危害这个的事务生命周期，和更新交易的轨迹。我们可以实现一个合适的try-finally代码块，但是每一次有人接触这个代码，我们必须重复的确保它没有被破坏。

幸好，我们还有另一种选择。我们可以去除工厂，把这些代码加在它的头部。与接收一个事务相比，我们可以把处理的代码逻辑放在一个很好掌控的函数中，就像下面这样：
``` java
runWithinTransaction((Transaction transaction) -> {
//... operation to run within the transaction ...
});
```
一个简单的修改就节省了很多步骤。检查状态和更新交易记录的方法被抽象和封装在runWithinTransaction这个方法里面了。对于这个方法来说，我们把需要translation上下文环境的代码传递进去了。我们再也不用担心执行这些步骤了，并且异常也可以非常好的处理。这种规定的函数会帮我们解决所有的东西。我们将会在第五章节，学习如何使用lambda表达式来增强那些方法。
#### 扩展策略
策略在我们身边一直在增长，超出了它们被需要的程度。企业级的应用有要求拓展它们。
